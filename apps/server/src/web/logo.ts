/** The Naka mark. "Naka" is Hindi for a checkpoint, the barrier where things are inspected before they pass. */
const GRADIENT = `<linearGradient id="nkg" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#2b6cb0"/><stop offset="1" stop-color="#7c3aed"/></linearGradient>`;

const GLYPH = `
  <g fill="none" stroke="#fff" stroke-width="26" stroke-linecap="round" stroke-linejoin="round">
    <path d="M70 196V60"/>
    <path d="M186 196V60"/>
    <path d="M70 60l116 136"/>
  </g>
  <circle cx="70" cy="60" r="17" fill="#fff"/>
  <circle cx="186" cy="196" r="24" fill="#34d399" stroke="#fff" stroke-width="9"/>`;

/** Square app icon / favicon. */
...
export function logoMark(size = 22, className = "nk-mark"): string {
  return `<svg class="${className}" width="${size}" height="${size}" viewBox="0 0 256 256" aria-hidden="true"><defs>${GRADIENT}</defs><rect width="256" height="256" rx="56" fill="url(#nkg)"/>${GLYPH}</svg>`;
}
